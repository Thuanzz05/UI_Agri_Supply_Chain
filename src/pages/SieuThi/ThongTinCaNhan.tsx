...
import { PhoneOutlined, MailOutlined, HomeOutlined, SaveOutlined, ShoppingOutlined, FacebookOutlined, CameraOutlined } from '@ant-design/icons';
...
const ThongTinCaNhan: React.FC = () => {
  ...
  const loadUserInfo = async () => {
    try {
      ...
      // Gọi API lấy thông tin siêu thị
      const response = await apiService.getSupermarketById(user.maSieuThi);
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      await apiService.updateSupermarketInfo(user.maSieuThi, updateData);
            ...
            <div style={{ textAlign: 'center', padding: '20px 0' }}>
              <div style={{ position: 'relative', display: 'inline-block' }}>
                <Avatar 
                  ...
                  style={{ backgroundColor: imageBase64 || userInfo?.anhDaiDien ? 'transparent' : '#722ed1', marginBottom: 20 }}
                />
                <Upload
                  listType="picture"
                  fileList={fileList}
              ...
              <p style={{ color: '#8c8c8c', marginBottom: 4 }}>
                Mã siêu thị: {userInfo?.maSieuThi}
              </p>
          ...
          <Card title="Chỉnh sửa thông tin">
            ...
            >
              <Form.Item
                label="Tên siêu thị"
                name="tenSieuThi"
                rules={[{ required: true, message: 'Vui lòng nhập tên siêu thị!' }]}
              >
                <Input 
